...
| audio bed | voice delayed to start at 11.2s; **sleep lane only:** pink-noise rain (lowpass 1400 / highpass 300, amplitude 0.55) under the voice, `apad` + long tail for loopability |
...
| video | audio voice | aura | palette | rain bed | subs | thumbnail concept | video status |
|---|---|---|---|---|---|---|---|
| not-jealous (v2) | dusk ✓ | lava | blue | no | yes (cues exist) | iMessage bubbles: "to be clear i'm not jealous" / "but" / voice-note bubble | **done — archetype canon** |
| not-going-anywhere | dusk (re-voiced) | lava | blue | no | yes | shadowed-eyes anime still + quote | re-render after audio vetted |
| cant-sleep | dusk (re-voiced) | lava | **black** (black-screen-friendly) | **yes** + `--tail 300` | no (sleep = frictionless) | all-black minimal, faint core, "can't sleep?" | re-render after audio vetted |
| waited-up | veil (re-voiced) | lava | pink | no | yes | soft pink lamp-glow, "i waited up." | re-render after audio vetted |
| the-bodyguard | dusk (new) | lava | blue | no | yes | quote card: "six years. / i'm done pretending." | render after audio vetted |
